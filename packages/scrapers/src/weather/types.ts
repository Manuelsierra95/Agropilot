...
export type metarID =
  `${Uppercase<string>}${Uppercase<string>}${Uppercase<string>}${Uppercase<string>}`

export type weatherCloudId = regularID | metarID

type StringifyNumbers<T> = {
  // transform number of an object type to string
...
export interface DeviceInfo {
  device: {
    // self explanatory for most of things
  ...
  values: {
    // some of the data from weatherdata but not all - in string format
    temp: string
    hum: string
    dew: string
    wspdavg: string
